...
//   const [loading, setLoading] = useState(true);
//   const [totalOrdersCount, setTotalOrdersCount] = useState(0);
//   const [canceledQuantity, setCanceledQuantity] = useState(0);
//   const navigate = useNavigate();
...
//       // Calculate total canceled quantity
//       let totalCanceled = 0;
//       res.data.orders.forEach(order => {
//         if (order.status === 'Cancelled') {
//           order.orderItems?.forEach(item => {
//             totalCanceled += item.quantity || 0;
//           });
//         }
//       });
...
//       setOrders(res.data.orders);
//       setFilteredOrders(todaysOrders);
//       setTotalOrdersCount(res.data.orders.length);
//       setCanceledQuantity(totalCanceled);
//       calculateTodaysTotals(todaysOrders);
//     } catch (err) {
//       console.error("Failed to fetch orders:", err);
//     } finally {
...
const Dashboard = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const allOrdersRes = await axios.get("https://burka-2-1.onrender.com/order");
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      ...
      const canceledRes = await axios.get('https://burka-2-1.onrender.com/order/status/shipped');
      const canceledOrders = canceledRes.data.orders || [];
      
      // Fetch delivered orders
      const deliveredRes = await axios.get('https://burka-2-1.onrender.com/order/status/delivered');
      ...
      deliveredOrders.forEach(order => {
        order.orderItems?.forEach(item => {
          totalDelivered += item.quantity || 0;
        });
      });
      ...
      setCanceledQuantity(totalCanceled);
      setDeliveredQuantity(totalDelivered);
      calculateTodaysTotals(todaysOrders);
    } catch (err) {
      console.error("Failed to fetch orders:", err);
    } finally {
