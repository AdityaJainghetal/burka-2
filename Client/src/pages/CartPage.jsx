...
const CartPage = () => {
  ...
  const calculateTotal = () => {
    const subtotal = cart.reduce(
      (sum, item) => sum + item.product.price * (editedQuantities[item._id] || 1),
      0
    );
  ...
  const handlePayment = async () => {
    ...
    const payload = {
      orderItems: cart.map(item => ({
        ...
        discountName: vendors.find(v => v._id === selectedVendor) || "",
...
<div className="flex items-center mt-2">
  {/* <button
    disabled={editedQuantities[item._id] <= 1}
    onClick={() =>
      handleQuantityChange(item._id, editedQuantities[item._id] - 1)
    }
    className="h-8 w-8 border rounded-l hover:bg-gray-50 disabled:opacity-50"
  >
    <Minus className="h-5 w-5 m-auto" />
  ...
  <input
    ...
    onChange={(e) => {
      ...
      handleQuantityChange(item._id, isNaN(value) || value < 1 ? 1 : value);
    }}
    min="1"
    className="h-8 w-16 text-center border-t border-b"
  />
    ...
    <Plus className="h-5 w-5 m-auto" />
                ...
                <div className="flex flex-col items-end justify-between">
                  <p className="font-medium text-lg">
                    ₹
                    {(
                      item.product.price * editedQuantities[item._id]
                    ).toFixed(2)}
                  </p>
                  <button
...
//   const calculateTotal = () => {
//     const subtotal = cart.reduce(
//       (sum, item) => sum + item.product.price * (editedQuantities[item._id] || 0),
//       0
//     );
//     const discAmt = (subtotal * discount) / 100;
//     return { 
